...
public class TaskController {
    ...
    @GetMapping
    @Operation(summary = "Retourne les taches", description = "Retourne une liste de tâche. " + "Si completed=true, retourne uniquement les tâches complétées. " + "Si completed=false, retourne uniquement les tâches incompletées. " + "Si absent retourne toutes les tâches.", parameters = {@Parameter(name = "completed", description = "Filtrer par tâches complétées ou non", examples = {@ExampleObject(name = "Toutes les tâches", value = ""), @ExampleObject(name = "Tâches complétées", value = "true"), @ExampleObject(name = "Tâches non complétées", value = "false")})}, responses = {@ApiResponse(responseCode = "200", description = "Liste des tâches retournée avec succès")})
    ...
    public ResponseEntity<TaskDto> createTaskById(@Valid @RequestBody TaskDto taskDto) {
        ...
        return ResponseEntity.created(URI.create("/tasks/" + created.getId())).body(TaskMapper.toDto(created));
    }
