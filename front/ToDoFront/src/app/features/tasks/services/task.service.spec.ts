...
describe('TaskService', () => {
  ...
  it('should fetch completed tasks', () => {
    ...
    const req = httpMock.expectOne(r => r.url === 'http://localhost:8080/api/tasks' && r.params.get('completed') === 'true');
    expect(req.request.method).toBe('GET');
    req.flush([]);
  });
  ...
  it('should post a new task', () => {
    const newTask: Task = { label: 'Nouvelle tâche', description: '', completed: false };
